...
## Open Questions
{Unresolved items that $bb-plan must address}

## Risks
{Top 2–3 risks worth watching}
```

**B. Spec file** → `plans/{slug}/spec.md` (from `.agents/skills/bb-brainstorm/references/spec-template.md`)
...
date: {YYYY-MM-DD}
```
This is the pointer file `$bb-journal` reads to locate the latest brainstorm without relying on in-context memory.
...
- `→ $bb-plan plans/{slug}/spec.md` — proceed to planning
- `→ $bb-journal` — archive, no plan yet
