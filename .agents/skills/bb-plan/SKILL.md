---
name: bb-plan
description: "Plan a feature or system before implementation. Use when the user says \"plan this\", \"I want to build X\", \"how do I implement Y\", or when $bb-brainstorm produces a spec.md. Always run before $bb-cook. Modes (pick one): --fast (skip all, instant plan), --hard (2 researchers + red-team + validate), --two (2 approaches → compare → pick → cook), --parallel (parallel-impl plan → bb:cook --parallel), --auto (full pipeline + auto-cook). Composable flags: --tdd, --no-task — propagate into the cook pipeline."
---

# bb:plan — Structured Planning Pipeline
...
| ------------ | -------------------------------- | ------------ | ----------- | -------------------------------- |
| `--fast`     | —                                | —            | —           | `$bb-cook --fast`                |
| `--hard`     | 2 researchers                    | ✓            | ✓ (wait)    | `$bb-cook --hard`                |
| `--two`      | 2 researchers (one per approach) | ✓ both plans | pick A or B | `$bb-cook [user-chosen mode]`    |
| `--parallel` | 2 researchers                    | ✓            | optional    | `$bb-cook --parallel`            |
...
If `--hard` / `--two` / `--parallel` and novel/ambiguous with no brainstorm report: "No brainstorm found. Run `$bb-brainstorm` first? [Y/n]" — if Yes, stop; if No, proceed.
...
| Mode         | Cook command                                                                                 |
| ------------ | -------------------------------------------------------------------------------------------- |
| `--fast`     | `$bb-cook --fast [--tdd] plans/{slug}/plan.md`                                               |
| `--hard`     | `$bb-cook --hard [--tdd] plans/{slug}/plan.md`                                               |
| `--two`      | `$bb-cook [--fast\|--hard] [--tdd] plans/{slug}/plan.md`                                     |
| `--parallel` | `$bb-cook --parallel [--tdd] plans/{slug}/plan.md`                                           |
| `--auto`     | Automatically invoke `$bb-cook --{detected-mode} plans/{slug}/plan.md` — no separate command |
