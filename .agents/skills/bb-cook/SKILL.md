...
# bb:cook — Structured Implementation Pipeline
...
- **`--parallel`** — phases have exclusive File Ownership (from `bb:plan --parallel`); auto-continue between phases (no per-phase review gate), full test + review at end
...
2. If none found → ask: "No plan found. Continue anyway? [y/N]" — if No, suggest `$bb-plan`
